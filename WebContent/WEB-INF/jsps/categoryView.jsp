 ...
 <!-- Google Fonts -->
    ...
    <link href="<c:url value="/css/themes/all-themes.css" />" rel="stylesheet" />
		...
		function editCheckbox(id,username,active) {
			...
			swal({
				 title: "Are you sure?",
			        text: "You want to edit the Active/Inactive Category?",
			        type: "warning",
				...
				function(isConfirm){
					 if (!isConfirm){ 
						...
						$.ajax({
							...
							url: "/LogManagement/editCategory.html?active="+active+"&username="+username+"&id="+id,
                            type: "GET",		
                            success: function () {
                            	
                	            location.href = "/LogManagement/categoryView.html?username="+username
                            },
							error: function () {
...
<body class="theme-red">
	...
	<div class="container-fluid">
		<div class="block-header">
                ...
                <button type="button" class="btn bg-red waves-effect" onclick="location.href='addCategory.html'">Add Category</button>
            </div>
                              ...
                              <input type="hidden" name="username" id="username2" value="">
                                ...
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <c:if test="${not empty listofcategory}">
		          						<c:forEach var="listofcategory" items="${listofcategory}" varStatus="i">
	                                        <tr>
	                                            <td><c:out value="${i.count}"></c:out></td>
	                                            <td><c:out value="${listofcategory.name}"></c:out></td>
	                                            
	                                            <td>
												<c:if test="${listofcategory.isActive eq 1 }">
	                                            	<div class="switch">
                                            			<label><input type="checkbox" id="checking" value="True" checked onchange="editCheckbox('${listofcategory.sid}',document.getElementById('username2').value, document.getElementById('checking').value)"><span class="lever switch-col-green"></span></label>
	                                        	</div>
	                                        	</c:if>
                                        		
                                        		<c:if test="${listofcategory.isActive eq 0 }">
	                                            	<div class="switch">
                                            			<label><input type="checkbox" id="checking2" value="False" onchange="editCheckbox('${listofcategory.sid}',document.getElementById('username2').value, document.getElementById('checking2').value)"><span class="lever switch-col-green"></span></label>
	                                        	</div>
	                                        	</c:if>                          
	                                            </td>
